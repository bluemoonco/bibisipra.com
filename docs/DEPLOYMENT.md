...
The report under `docs/` is internal project documentation and is not in the published directory. The public site has no application JavaScript, contact backend or analytics tag.
...
10. The existing Weebly contact backend is not part of this deployment. The new site uses Bibi's confirmed LinkedIn profile for inquiries and CV requests.
...
- Header navigation, anchors, skip link, FAQ details and LinkedIn links work on desktop and mobile.
